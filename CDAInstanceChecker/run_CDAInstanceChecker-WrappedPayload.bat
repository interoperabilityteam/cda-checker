REM: **************************************************************
REM: run_CDAInstanceChecker-WrappedPayload.bat
...
REM:     - input_wrapped - directory of wrapped on-the-wire CDA instances to be used as input for the CDA Instance Checker
REM:     - input_extracted - directory where extracted on-the-wire payloads will be written following stripping of HL7 wrappers
...
REM:     - xsl_extract - XSLT file to use to extract payload from HL7 wrapped instance
...
java -classpath lib\saxonhe9-3-0-5j\saxon9he.jar net.sf.saxon.Transform -s:%input_wrapped% -xsl:%xsl_extract% -o:%input_extracted%  1>>%log% 2>&1


ECHO. >>%log%


ECHO STEP 2: Check the on-the-wire instances for validity against the XML Schema and Schematron >>%log%
  IF %schema_otw%==null (
    java -classpath lib\xerces-2_9_0\xercesImpl.jar;lib\cdaInstanceChecker-1_0\cdaInstanceChecker.jar org.net.nhs.hl7.cdaInstanceChecker.CDAInstanceChecker %input_extracted% %report_xml_otw% %schematron_otw% 1>>%log% 2>&1
  ) ELSE (
    java -classpath lib\xerces-2_9_0\xercesImpl.jar;lib\cdaInstanceChecker-1_0\cdaInstanceChecker.jar org.net.nhs.hl7.cdaInstanceChecker.CDAInstanceChecker %input_extracted% %report_xml_otw% %schematron_otw% %schema_otw% 1>>%log% 2>&1
  )
...
ECHO STEP 3: Transform the on-the-wire instances to templated instances >>%log%
  java -classpath lib\saxonhe9-3-0-5j\saxon9he.jar net.sf.saxon.Transform -s:%input_extracted% -xsl:%xsl_transform% -o:%output_templated% 1>>%log% 2>&1


ECHO. >>%log%


ECHO STEP 4: Check the newly created templated instances for validity against the XML Schema and Schematron >>%log%
