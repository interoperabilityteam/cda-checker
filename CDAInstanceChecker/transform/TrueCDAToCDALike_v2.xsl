	...
	<!--
		...
		Updated Prashant Trivedi v2.2 04/03/2014
	-->
...
<!--	</xsl:template>-->
	...
	<xsl:template
		...
		not(
		contains(parent::node()/element(),'telecom') or
		contains(parent::node()/preceding-sibling::h:templateId/@extension,'ssessment') or
	...
	<!--<xsl:template match="h:structuredBody/h:component[child::npfitlc:contentId]">
		...
		<xsl:choose>
			<xsl:when test="contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT010007GB02')
									...
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT000021GB01')">
