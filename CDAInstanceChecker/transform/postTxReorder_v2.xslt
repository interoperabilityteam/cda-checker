...
<!--	<xsl:comment>RIM class is <xsl:value-of select="$rimClass">
	...
	<xsl:template match="h:*|npfitlc:*" mode="alpha-order">
		<xsl:copy>
			...
			<xsl:apply-templates select="*[not(@typeCode) and not(@classCode) and not(local-name(.) = 'languageCommunication')]" mode="alpha-order">
				<xsl:sort select="local-name(.)" case-order="lower-first"/>
			</xsl:apply-templates>
	...
	<xsl:template match="h:tel|h:value|h:effectiveTime|h:useablePeriod|h:code|h:name|h:time|h:text|h:addr|h:repeatNumber" mode="alpha-order">
        <xsl:copy-of select="."/>
    </xsl:template>
