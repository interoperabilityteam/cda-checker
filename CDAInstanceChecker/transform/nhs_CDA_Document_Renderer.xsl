...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3" xmlns:npfitlc="NPFIT:HL7:Localisation" xmlns:n2="urn:hl7-org:v3/meta/voc" xmlns:voc="urn:hl7-org:v3/voc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="n1 n2 voc npfitlc xsi">
	...
	<xsl:template name="CodedEntryMatch">
		...
		<xsl:variable name="v_codedentry" select="$temp_1/child::node()[child::n1:templateId[contains(@extension,'COCD_TP')]]"/>
	...
	<xsl:template name="patientBanner">
		<div id="patientBanner" class="banner">
			<table>
				<tbody>
					<tr>
						<td class="label">Patient demographics</td>
					</tr>
					<tr>
						<td class="label">Patient name</td>
						<td class="label">Date of birth</td>
						<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/npfitlc:deceasedTime">
							<td class="label">Date of death</td>
						</xsl:if>
						<td class="label">Gender</td>
						<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/n1:ethnicGroupCode">
							<td class="label">Ethinicity</td>
						</xsl:if>						
						<td class="label">NHS number</td>
						<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:id[@root='2.16.840.1.113883.2.1.3.2.4.18.24']">
							<td class="label">Other identifier</td>
						</xsl:if>
					</tr>
					...
					<tr>
						...
						<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/npfitlc:deceasedTime">
							<td>
								<xsl:call-template name="formatDate">
									<xsl:with-param name="date" select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/npfitlc:deceasedTime/@value"/>
								</xsl:call-template>							
							</td>
						</xsl:if>
						...
						<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/n1:ethnicGroupCode">
							<td>
								<xsl:value-of select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/n1:ethnicGroupCode/@displayName"/>
							</td>
						</xsl:if>						
						<td>
						...
						<td>
							<xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:id[@root='2.16.840.1.113883.2.1.3.2.4.18.24']">
								<!-- Local ID -->
								<span class="label">Local identifier - </span>
							...
							<td>
								...
								<xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:patient/n1:languageCommunication">
									<p>
										...
										<xsl:if test="./n1:proficiencyLevelCode">
											<xsl:text/>
											<xsl:value-of select="./n1:modeCode/@displayName"/>
											<xsl:text>) </xsl:text>
										</xsl:if>
										<!--			<xsl:if test="./n1:preferenceInd">
											<xsl:text> Preferred (</xsl:text>
											<xsl:value-of select="./n1:preferenceInd/@value"/>
											<xsl:text>)</xsl:text>
										</xsl:if>-->
				...
				<tbody>
					...
					<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:providerOrganization">
						<tr>
							...
							<td>
								<xsl:for-each select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:providerOrganization/n1:addr">
									<p class="label">
										<xsl:choose>
											<xsl:when test="@use='TMP'">Temporary Address</xsl:when>
											<xsl:when test="@use='PST'">Postal Address</xsl:when>
						...
						<tr>
							<td>
								<p class="label">GP Practice Code</p>
								<xsl:value-of select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:providerOrganization/n1:id/@extension"/>
							</td>
						</tr>
					</xsl:if>
				</tbody>
							...
							<tr>
								...
								<td class="titlebar">
									<xsl:if test="n1:assignedEntity/n1:assignedPerson/n1:name">
										...
										<xsl:if test="n1:assignedEntity/n1:representedOrganization/n1:name">
											<!--PRTR1 code added to get the org name rendered-->
											<xsl:text>, </xsl:text>
											<xsl:call-template name="getName">
  ...
  -->
								...
								<xsl:when test="@typeCode='PRF' and not(preceding-sibling::*/@typeCode='PRF')">
									<xsl:choose>
										...
										<xsl:otherwise>
											<span class="label">
												<xsl:text>Performed by </xsl:text>
												<!--PRTR1 Changed with comment from KZS-->
											</span>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:when>
	...
	<xsl:template name="translateLanguageCode">
		...
		<xsl:choose>
			...
			<xsl:when test="$languageCode='cu'">
				<xsl:text>Church Slavonic</xsl:text>
			</xsl:when>
			...
			<xsl:when test="$languageCode='nr'">
				<xsl:text>South Ndebele</xsl:text>
			</xsl:when>
			<xsl:when test="$languageCode='nd'">
				<xsl:text>North Ndebele</xsl:text>
			</xsl:when>
	...
	<xsl:template name="translateStatusCode">
		...
		<xsl:choose>
			...
			<xsl:when test="$statusCode='02'">
				<xsl:text>Number present but not traced</xsl:text>
			</xsl:when>
			<xsl:when test="$statusCode='03'">
				<xsl:text>Trace required</xsl:text>
			</xsl:when>
			<xsl:when test="$statusCode='04'">
				<xsl:text>Trace attempted</xsl:text>
			</xsl:when>
			<xsl:when test="$statusCode='05'">
				<xsl:text>Trace needs to be resolved</xsl:text>
			</xsl:when>
			<xsl:when test="$statusCode='06'">
				<xsl:text>Trace in progress</xsl:text>
			</xsl:when>
			<xsl:when test="$statusCode='08'">
				<xsl:text>Trace postponed (baby &lt; 6 weeks old)</xsl:text>
			</xsl:when>
			<xsl:otherwise>
